...
# EXPLORE: opport-magnitude — 幅度模型 (绝对值回归, 不做排名)
GLOBAL_OPPORT_MAG_PKG = None
GLOBAL_RISK_MAG_PKG = None
opport_mag_model = None
opport_mag_features = None
risk_mag_model = None
risk_mag_features = None

def load_magnitude_models(opport_pkl=None, riskmag_pkl=None):
    global GLOBAL_OPPORT_MAG_PKG, GLOBAL_RISK_MAG_PKG
    global opport_mag_model, opport_mag_features, risk_mag_model, risk_mag_features
    if opport_pkl and os.path.exists(opport_pkl):
        GLOBAL_OPPORT_MAG_PKG = joblib.load(opport_pkl)
        opport_mag_model = GLOBAL_OPPORT_MAG_PKG['model']
        opport_mag_features = GLOBAL_OPPORT_MAG_PKG['features']
        print(f"机会幅度模型已加载: {opport_pkl}")
    if riskmag_pkl and os.path.exists(riskmag_pkl):
        GLOBAL_RISK_MAG_PKG = joblib.load(riskmag_pkl)
        risk_mag_model = GLOBAL_RISK_MAG_PKG['model']
        risk_mag_features = GLOBAL_RISK_MAG_PKG['features']
        print(f"风险幅度模型已加载: {riskmag_pkl}")
...
# EXPLORE: opport-magnitude — 可配置参数, 供扫描脚本调参
RISK_MAG_SELL_THRESHOLD = 2.5   # 风险幅度卖出阈值: risk_mag > this → sell
...
class AKShareChipDataSource(DataSource):
    ...
    def _fetch_data(self, symbols, start_date, end_date, timeframe, adjust="qfq", online=False,
        ...
        # EXPLORE: opport-magnitude — 幅度模型推断
        if opport_mag_model is not None:
            opport_input_features = [f for f in opport_mag_features]
            final_df_smooth['opport_mag'] = opport_mag_model.predict(final_df_smooth[opport_input_features])
        if risk_mag_model is not None:
            risk_input_features = [f for f in risk_mag_features]
            final_df_smooth['risk_mag'] = risk_mag_model.predict(final_df_smooth[risk_input_features])
...
def check_buy_eligibility_and_score(ctx, daily_env):
    ...
    # 3. 幅度门槛: 机会幅度模型预测不达标的直接排除 (绝对值回归, 避免矮子里选将军)
    opport_mag_ok = True
    if hasattr(ctx, 'opport_mag') and ctx.opport_mag is not None:
        opport_mag_val = ctx.opport_mag[-1] if isinstance(ctx.opport_mag, (list, np.ndarray)) else ctx.opport_mag
        opport_mag_ok = opport_mag_val > 0  # target_val > 0 → 预期正收益
    
    # 4. 筹码形态与个股基本面精筛 (幅度模型覆盖后, 芯片条件大幅放松)
    is_chip_ready = (
        getattr(ctx, 'is_profit_ok', False)[-1] and
        ml_rank < ml_threshold and
        # risk_ml_rank > 0.02 and
        opport_mag_ok and
        bias_con and
...
def before_exec_fn(ctx_map):
    ...
    # 自适应: 高分候选多 → +1只
    if sum(1 for _, r in daily_candidates if r < 0.02) >= 3:
        buy_quota = min(6, buy_quota + 1)
...
def chip_strategy(ctx):
    ...
    # -------------------------------------------------------------------------
        ...
        # EXPLORE: opport-magnitude — 风险幅度门槛
        if hasattr(ctx, 'risk_mag') and ctx.risk_mag is not None:
            risk_mag_val = ctx.risk_mag[-1] if isinstance(ctx.risk_mag, (list, np.ndarray)) else ctx.risk_mag
            if not should_sell and risk_mag_val > RISK_MAG_SELL_THRESHOLD:
                should_sell = True
                sell_reason = "Risk_Magnitude_High"
        ...
        if not should_sell and curr_pnl > 0.06:
            ...
            if ml_rank < 0.01:
                mult = 3
            if hasattr(ctx, "opport_mag") and ctx.opport_mag is not None and ctx.opport_mag[-1] > 0.015:
                mult += 0.8  # 极度宽容，防止被洗掉
